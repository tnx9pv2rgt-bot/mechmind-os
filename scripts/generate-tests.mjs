...
async function main() {
  ...
  // ─── Step 5: Genera spec.ts per ogni service ──────────────────────────────
  ...
  console.error(`\n   Generando ${allServices.length} spec file:`);

  for (let i = 0; i < allServices.length; i++) {
    const servicePath = allServices[i];
    const serviceBaseName = relative(BACKEND_SRC, servicePath).replace(/\.service\.ts$/, '.service');
    const specPath = servicePath.replace(/\.service\.ts$/, '.service.spec.ts');

    console.error(`   [${i + 1}/${allServices.length}] ${relative(ROOT, specPath)}...`);
    ...
    try {
      ...
      // Report cache benefit
      if (i === 0 && cacheWrite > 0) {
        console.error(`       ⚡ Cache write su sourceBlock: ${cacheWrite.toLocaleString()} token`);
      }
      if (i > 0 && cacheRead > 0) {
        const saved = Math.round(cacheRead * 0.9);
        console.error(`       ✅ Cache read: -${saved.toLocaleString()} token (90% risparmio)`);
      }
      ...
      writeFileSync(specPath, textBlock.text + '\n');
      console.error(`       ✅ Scritto`);
    } catch (err) {
      console.error(`       ❌ Errore: ${err.message}`);
    }
  }

  // ─── Step 6: Esegui Jest e raccogli coverage ──────────────────────────────

  console.error(`\n🧪 Eseguendo Jest per modulo "${moduleName}"...`);
  ...
  // Mostra ultimi 15 righe di output Jest
  console.error('\n   Ultime righe Jest:');
  ...
  // ─── Step 7: Aggiorna MODULI_NEXO.md ──────────────────────────────────────

  console.error(`\n📝 Aggiornamento MODULI_NEXO.md...`);
  const esito = jestOutput.includes('Tests:') && jestOutput.includes('passed') ? '✅ Testato' : '⏳ Generato';
  ...
  console.error(`\n✅  Completato: ${moduleName} — ${coverage.stmt} stmt / ${coverage.branch} branch — ${esito}`);
}
