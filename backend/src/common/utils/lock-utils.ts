...
export function validateLockOrder(locks: LockIdentifier[]): LockIdentifier[] {
  ...
  const isOrdered = locks.every((lock, index) => 
    lock.tenantId === sorted[index].tenantId && 
 ...
 */
export function computeLockIdSafe(tenantId: string, slotId: string, existingLocks: Set<bigint> = new Set()): bigint {
