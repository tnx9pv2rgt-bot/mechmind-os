...
**Ultimo aggiornamento:** 2026-05-19 **Branch corrente:** `qa/booking-coverage`
...
| 2026-05-16 | LOGIN AUDIT 2026: endpoint GET /v1/auth/tenant-by-email + proxy frontend. Fix 7 bug: BUG-1 errori inglese→italiano, BUG-2 focus email post-consent, BUG-3 focus pw step2 250ms, BUG-4 redirect autenticato (fix r.ok→data.user), BUG-5 cookie secure prod già ok, BUG-6 throttle by-design, BUG-7 Apple OAuth nascosta. Throttle bypass localhost dev (app.module.ts). 34/34 Playwright ✅ + 3/3 curl ✅. | login/page.tsx, social-buttons.tsx, app.module.ts, auth.service.spec.ts (+7t), auth.controller.spec.ts (+4t), e2e/auth/login-audit-2026.spec.ts (NEW 34t), pw-audit.config.ts (NEW) | BUG-1 ✅ BUG-2 ✅ BUG-3 ✅ BUG-4 ✅ BUG-7 ✅     | —                                     |
| 2026-05-19 | Audit CLAUDE.md+regole: automation-status.md (373r) e test-quality-gates.md (318r) spostati in docs/ (fuori contesto permanente). coverage-standard.md da 131→26r. frontend.md da 32→30r (Prettier +6 blank). Contesto permanente: 1029r→237r (-792r, -77%). Sincronizzata PROJECT-MEMORY: Fase 0 IN CORSO 50%, PROSSIMA AZIONE aggiornata, contraddizione roadmap/cronologia risolta.                    | .claude/rules/\*, .claude/teams/PROJECT-MEMORY.md                                                                                                                                   | —                                                | —                                     |
...
| ------------------------- | -------- | --------------- | ----------------------------------------------------------------------------------- | -------------------------- |
| **0: Fondamenta**         | 2 sett   | ⏳ IN CORSO 50% | 30 hook spec mancanti, jest/jsdom config da completare, 10 moduli E2E da aggiungere | test-runner + ts-fixer     |
...
### PROSSIMA AZIONE IMMEDIATA (aggiornata 2026-05-19)

**Fase 0 — Fondamenta** — IN CORSO 50%

Completato ✅: rentri.spec.ts (13t), billing.spec.ts (13t), useIsClient.test.ts
(22t), useBilling.test.ts (10t), public-token CEILING_ACCEPTED.

Da completare ⏳:

```
1. cd frontend && npx jest --version  (verifica se già installato)
   Se mancante: npm install --save-dev jest jest-environment-jsdom @testing-library/react @testing-library/jest-dom
2. Scrivere spec per i 30 hook rimanenti (priorità: useBookings, useInvoices, useCustomers)
3. Aggiungere E2E per 10 moduli mancanti: admin, voice, workflows, production-board,
   search, calendar, diagnostics, payments, payroll, audit-logs
4. Aggiornare COVERAGE-REAL.md e questo file
```
