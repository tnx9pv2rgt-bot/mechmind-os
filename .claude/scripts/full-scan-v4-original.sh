...
add_sarif_finding() {
    ...
    local rule_exists
    rule_exists=$(jq --arg id "$rule_id" '[.runs[0].tool.driver.rules[]? | select(.id == $id)] | length' "$SARIF_REPORT")
    if [[ "$rule_exists" == "0" ]]; then
        jq --arg id "$rule_id" --arg msg "$message" \
           '.runs[0].tool.driver.rules += [{"id": $id, "shortDescription": {"text": $msg}}]' \
           "$SARIF_REPORT" > "$SARIF_REPORT.tmp" && mv "$SARIF_REPORT.tmp" "$SARIF_REPORT"
    fi

    jq --arg rule "$rule_id" --arg msg "$message" --arg file "$file" \
       --argjson line "$line" --argjson col "$col" --arg level "$level" \
       '.runs[0].results += [{
         "ruleId": $rule,
         "message": {"text": $msg},
         "level": $level,
...
fase_1_semgrep() {
    ...
    if [[ -f "$semgrep_out" ]]; then
        jq -c '.results[]?' "$semgrep_out" 2>/dev/null | while read -r finding; do
            ...
            add_sarif_finding "$severity" "$rule" "$msg" "$file" "$line"
        done
...
fase_2_codeql() {
    ...
    codeql database create "$codeql_db" --language=javascript --source-root="$PROJECT_ROOT" --quiet 2>/dev/null || {
    ...
    if command -v trivy &>/dev/null; then
        ...
        if [[ -f "$trivy_out" ]]; then
            jq -c '.Results[]? | select(.Secrets) | .Secrets[]?' "$trivy_out" 2>/dev/null | while read -r secret; do
                ...
                add_sarif_finding "CRITICAL" "$rule" "$msg" "$file" "$line"
            done
        fi
        ...
        if command -v trufflehog &>/dev/null; then
            ...
            if [[ -f "$truffle_out" ]]; then
                jq -c 'select(.DetectorName != "")' "$truffle_out" 2>/dev/null | while read -r finding; do
                    ...
                    add_sarif_finding "CRITICAL" "trufflehog-$detector" "Secret $detector detected in git history" "$file" "$line"
                done
            fi
        else
    ...
    if command -v trivy &>/dev/null; then
        ...
        if [[ -f "$SBOM_PATH" ]]; then
            ...
            if [[ -f "$vuln_out" ]]; then
                jq -c '.Results[]? | select(.Vulnerabilities) | .Vulnerabilities[]?' "$vuln_out" 2>/dev/null | while read -r vuln; do
                    ...
                    add_sarif_finding "$severity" "$cve" "$msg" "package.json" 1
                done
            fi
        fi
    else
        ...
        if [[ -f "$audit_out" ]]; then
            jq -c '.vulnerabilities? // {} | to_entries[]?' "$audit_out" 2>/dev/null | while read -r entry; do
                ...
                add_sarif_finding "$severity" "npm-audit-$pkg" "$msg" "package.json" 1
            done
        fi
    fi
}
...
fase_5_backend_typescript() {
    ...
    npx tsc --noEmit --pretty false 2>/dev/null | grep "error TS" | while read -r line; do
    ...
    npx eslint src --max-warnings 0 --format json --quiet > "$eslint_out" 2>/dev/null || true
        ...
        jq -c '.[]?' "$eslint_out" 2>/dev/null | while read -r obj; do
            ...
            if [[ -n "$messages" ]]; then
                echo "$messages" | while read -r msg; do
                    ...
                    severity=$(echo "$msg" | jq -r '.severity == 2 ? "HIGH" : "MEDIUM"')
                    add_sarif_finding "$severity" "$rule" "[$rule] $text" "backend/$file" "$lineno"
                done
            fi
        done
    fi
    ...
    grep -rIn "@ts-ignore\|@ts-expect-error" src --include="*.ts" 2>/dev/null | head -20 | while read -r line; do
        local file lineno
...
fase_6_backend_coverage() {
    ...
    if [[ -f "vitest.config.ts" ]]; then
        ...
        npx jest --coverage --coverageReporters=json-summary --coverageDirectory="$jest_out" --silent 2>/dev/null || true
...
fase_7_backend_resilience() {
    ...
    grep -rIn "catch\s*[(][^)]*[)]\s*[{]\s*[}]" src --include="*.ts" 2>/dev/null | while read -r line; do
        local file lineno
        file=$(echo "$line" | cut -d: -f1)
        lineno=$(echo "$line" | cut -d: -f2)
    ...
    grep -rIn "console\.log\|console\.warn\|console\.error" src --include="*.ts" 2>/dev/null | while read -r line; do
        local file lineno
        file=$(echo "$line" | cut -d: -f1)
        lineno=$(echo "$line" | cut -d: -f2)
...
fase_9_frontend_eslint() {
        ...
        jq -c '.[]?' "$eslint_out" 2>/dev/null | while read -r obj; do
            ...
            if [[ -n "$messages" ]]; then
                echo "$messages" | while read -r msg; do
                    ...
                    text=$(echo "$msg" | jq -r '.message // "ESLint issue"')
                    severity=$(echo "$msg" | jq -r '.severity == 2 ? "HIGH" : "MEDIUM"')
                    add_sarif_finding "$severity" "$rule" "[$rule] $text" "frontend/$file" "$lineno"
                done
            fi
        done
    fi
    ...
    if ! npx eslint --print-config src/main.tsx 2>/dev/null | jq -e '.rules["react-hooks/rules-of-hooks"]' &>/dev/null; then
        ...
        add_sarif_finding "MEDIUM" "react-hooks-missing" "react-hooks/rules-of-hooks non configurato in ESLint" "frontend/.eslintrc" 1
    fi
    ...
    if command -v npx knip &>/dev/null; then
        ...
        if [[ -f "$knip_out" ]]; then
            jq -c '.issues?.dependencies? // [] | .[]?' "$knip_out" 2>/dev/null | while read -r issue; do
                ...
                add_sarif_finding "LOW" "knip-dead-code" "$msg" "frontend/$file" 1
            done
        fi
    fi
}
...
fase_10_frontend_vitest() {
    ...
    npx vitest run --reporter=json --outputFile="$vitest_out" --silent 2>/dev/null || true
    ...
    if [[ -f "$vitest_out" ]]; then
        jq -c '.testResults[]?' "$vitest_out" 2>/dev/null | while read -r result; do
            ...
            if [[ "$failed" -gt 0 ]] 2>/dev/null; then
                add_sarif_finding "HIGH" "vitest-failed" "$failed test falliti" "frontend/$file" 0
            fi
        done
    fi

    local coverage_out="$REPORT_DIR/frontend-coverage.json"
    npx vitest run --coverage --reporter=json --outputFile="$coverage_out" --silent 2>/dev/null || true
...
fase_11_playwright() {
    ...
    if [[ -f "$pw_out" ]]; then
        jq -c '.suites[]? | .specs[]? | select(.ok == false)' "$pw_out" 2>/dev/null | while read -r spec; do
            ...
            add_sarif_finding "HIGH" "playwright-failed" "Test E2E fallito: $file" "frontend/e2e/$file" 0
        done
    fi
    ...
    if grep -rq "axe" e2e --include="*.spec.ts" 2>/dev/null; then
        ...
        if [[ -f "$axe_out" ]]; then
            jq -c '.suites[]? | .specs[]? | select(.ok == false)' "$axe_out" 2>/dev/null | while read -r spec; do
                local file
                file=$(echo "$spec" | jq -r '.title // "unknown"')
                add_sarif_finding "MEDIUM" "axe-violation" "WCAG violation: $file" "frontend/e2e/$file" 0
            done
        fi
    else
    ...
    if command -v lychee &>/dev/null; then
        ...
        if [[ -f "$lychee_out" ]]; then
            jq -c '.fail_map? | to_entries[]?' "$lychee_out" 2>/dev/null | while read -r entry; do
                ...
                add_sarif_finding "HIGH" "dead-link" "Dead link: $url in $file" "frontend/$file" 1
            done
        fi
    else
    ...
    if command -v npx spectral &>/dev/null; then
        ...
        if [[ -f "$spectral_out" ]]; then
            jq -c '.[]?' "$spectral_out" 2>/dev/null | while read -r issue; do
                ...
                severity=$(echo "$issue" | jq -r '.severity == 0 ? "HIGH" : (.severity == 1 ? "MEDIUM" : "LOW")')
                add_sarif_finding "$severity" "spectral-lint" "$msg" "backend/openapi.yaml" "$line"
            done
        fi
    fi
...
fase_15_slsa_report() {
    ...
    jq --arg end "$end_time" '.runs[0].invocations[0].endTimeUtc = $end' "$SARIF_REPORT" > "$SARIF_REPORT.tmp" && mv "$SARIF_REPORT.tmp" "$SARIF_REPORT"
