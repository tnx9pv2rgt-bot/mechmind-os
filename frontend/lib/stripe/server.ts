...
export async function getOrCreateCustomer(params: {
  ...
  if (stripeCustomerId) {
    try {
      ...
      console.info('Customer not found, creating new one');
    }
  }
...
export async function toggleAiAddon(
  ...
  const aiItem = subscription.items.data.find(item => item.price.id === STRIPE_PRICE_IDS.aiAddon);
 ...
 */
export function handleStripeError(error: unknown): StripeServerError {
  if (error instanceof Stripe.errors.StripeError) {
    return new StripeServerError(error.message, error.code, error);
  }

  if (error instanceof Error) {
    return new StripeServerError(error.message, 'unknown_error');
  }

  return new StripeServerError(String(error), 'unknown_error');
}
