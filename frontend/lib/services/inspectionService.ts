...
export async function getInspectionById(id: string): Promise<Inspection> {
  ...
  try {
    ...
    const res = await api.get<{ data: Inspection | null }>(`/inspections/${id}`);
    const inspection = res.data?.data ?? null;
...
export async function listInspections(
  ...
  try {
    ...
    const res = await api.post<PaginatedInspections>('/inspections/list', params);
...
export async function deleteInspection(
  ...
  try {
    ...
    const res = await api.delete<{ success: boolean; deletedAt: Date }>(`/inspections/${id}`, {
      deletedBy,
    });
    const result = res.data;
...
export async function submitInspection(id: string): Promise<Inspection> {
  ...
  try {
    ...
    const res = await api.post<Inspection>(`/inspections/${id}/submit`);
    const inspection = res.data;
...
export function calculateOverallScore(inspection: Inspection): InspectionScoreBreakdown {
  ...
  for (const item of inspection.items) {
    ...
    if (item.status === 'ISSUE_FOUND' && item.severity) {
      ...
      categoryDeductions[category] = Math.min(categoryDeductions[category] + deduction, 9);
    }
  }
  ...
  const overall =
    Math.round(
      (mechanical * categories.mechanical.weight +
        ...
        safety * categories.safety.weight) *
        10
    ) / 10;
...
export function canEditInspection(inspection: Inspection): boolean {
  return !inspection.isDeleted && !['APPROVED', 'DECLINED', 'ARCHIVED'].includes(inspection.status);
}
...
export function canSubmitInspection(inspection: Inspection): boolean {
  return !inspection.isDeleted && ['IN_PROGRESS', 'PENDING_REVIEW'].includes(inspection.status);
}
...
export async function addFinding(
  ...
  try {
    const res = await api.post<InspectionFinding>(`/inspections/${inspectionId}/findings`, data);
...
export async function submitCustomerApproval(
  ...
  try {
    const res = await api.post<{ success: boolean }>(`/inspections/${inspectionId}/approve`, data);
    const result = res.data;
