...
export const itTemplates: Record<string, TemplateFunction> = {
  ...
  INVOICE_READY: (v) =>
    `Ciao ${v.customerName}, fattura ${v.invoiceNumber || ''} pronta. Importo: ${v.amount || 'N/D'}. Visualizza: ${v.link || 'https://mechmind.it/portal'}`,

  MAINTENANCE_DUE: (v) =>
    `Ciao ${v.customerName}, ${v.service || 'manutenzione'} dovuta tra ${v.days || 'pochi'} giorni per il tuo veicolo${v.vehiclePlate ? ` ${v.vehiclePlate}` : ''}. Prenota: ${v.link || 'https://mechmind.it/portal'}`,
  ...
  PAYMENT_REMINDER: (v) =>
    `Ciao ${v.customerName}, promemoria pagamento fattura ${v.invoiceNumber || ''}${v.amount ? ` di ${v.amount}` : ''}. Paga qui: ${v.link || 'https://mechmind.it/portal'}`,
};
...
export const itTemplatesExtended = {
  ...
  BOOKING_REMINDER_LONG: (v: TemplateVariables) =>
    `Ciao ${v.customerName},\n\nTi ricordiamo che hai un appuntamento prenotato per domani:\n📅 Data: ${v.date}\n⏰ Ora: ${v.time}\n📍 Location: ${v.location || v.workshopName || 'Il nostro centro'}\n🔢 Codice: ${v.bookingCode}\n\nPer modifiche o cancellazioni: ${v.link || 'https://mechmind.it/portal'}\n\nGrazie,\nTeam MechMind`,

  INSPECTION_REPORT_READY: (v: TemplateVariables) =>
    `Ciao ${v.customerName}, il report dell'ispezione del tuo veicolo${v.vehiclePlate ? ` ${v.vehiclePlate}` : ''} è pronto!${v.score ? ` Valutazione complessiva: ${v.score}/10` : ''} Visualizza il report completo: ${v.link || 'https://mechmind.it/portal'}`,
  ...
  SPECIAL_OFFER: (v: TemplateVariables) =>
    `Ciao ${v.customerName}, abbiamo un'offerta speciale per te! ${v.service || 'Manutenzione'} a prezzo scontato. Prenota entro ${v.days || '7'} giorni: ${v.link || 'https://mechmind.it/portal'}`,

  FEEDBACK_REQUEST: (v: TemplateVariables) =>
    `Ciao ${v.customerName}, grazie per aver scelto ${v.workshopName || 'MechMind'}! Ci piacerebbe sapere la tua opinione. Lascia una recensione: ${v.link || 'https://mechmind.it/feedback'}`,
};
