...
export const enTemplates: Record<string, TemplateFunction> = {
  ...
  INVOICE_READY: (v) =>
    `Hi ${v.customerName}, invoice ${v.invoiceNumber || ''} is ready. Amount: ${v.amount || 'N/A'}. View: ${v.link || 'https://mechmind.io/portal'}`,

  MAINTENANCE_DUE: (v) =>
    `Hi ${v.customerName}, ${v.service || 'maintenance'} due in ${v.days || 'a few'} days for your vehicle${v.vehiclePlate ? ` ${v.vehiclePlate}` : ''}. Book: ${v.link || 'https://mechmind.io/portal'}`,
  ...
  PAYMENT_REMINDER: (v) =>
    `Hi ${v.customerName}, payment reminder for invoice ${v.invoiceNumber || ''}${v.amount ? ` of ${v.amount}` : ''}. Pay here: ${v.link || 'https://mechmind.io/portal'}`,
};
...
export const enTemplatesExtended = {
  ...
  BOOKING_REMINDER_LONG: (v: TemplateVariables) =>
    `Hi ${v.customerName},\n\nJust a reminder that you have an appointment scheduled for tomorrow:\n📅 Date: ${v.date}\n⏰ Time: ${v.time}\n📍 Location: ${v.location || v.workshopName || 'Our center'}\n🔢 Code: ${v.bookingCode}\n\nTo modify or cancel: ${v.link || 'https://mechmind.io/portal'}\n\nThanks,\nMechMind Team`,

  INSPECTION_REPORT_READY: (v: TemplateVariables) =>
    `Hi ${v.customerName}, the inspection report for your vehicle${v.vehiclePlate ? ` ${v.vehiclePlate}` : ''} is ready!${v.score ? ` Overall rating: ${v.score}/10` : ''} View full report: ${v.link || 'https://mechmind.io/portal'}`,
  ...
  SPECIAL_OFFER: (v: TemplateVariables) =>
    `Hi ${v.customerName}, we have a special offer for you! ${v.service || 'Maintenance'} at a discounted price. Book within ${v.days || '7'} days: ${v.link || 'https://mechmind.io/portal'}`,

  FEEDBACK_REQUEST: (v: TemplateVariables) =>
    `Hi ${v.customerName}, thank you for choosing ${v.workshopName || 'MechMind'}! We'd love to hear your feedback. Leave a review: ${v.link || 'https://mechmind.io/portal'}`,
};
