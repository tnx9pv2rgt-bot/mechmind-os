...
function parseDelay(delay: string): number {
  ...
  switch (unit) {
    case 'd':
      return numValue * 24 * 60 * 60 * 1000; // giorni
    case 'h':
      return numValue * 60 * 60 * 1000; // ore
    case 'm':
      return numValue * 60 * 1000; // minuti
    default:
...
export function scheduleJob(
  ...
  console.info(`[Scheduler] Job ${jobId} scheduled for ${job.scheduledAt.toISOString()}`);
 ...
 */
export function scheduleStageReminder(customerId: string, stage: ProfilingStageId): string | null {
...
export function cancelScheduledJob(jobId: string): boolean {
  ...
  if (existed) {
    console.info(`[Scheduler] Job ${jobId} cancelled`);
  }
...
export function cancelAllReminders(customerId: string): number {
  ...
  console.info(`[Scheduler] Cancelled ${cancelledCount} reminders for customer ${customerId}`);
...
export function processExpiredJobs(): ScheduledJob[] {
  ...
  for (const [jobId, job] of scheduledJobs.entries()) {
    if (job.scheduledAt <= now) {
      ...
      console.info(`[Scheduler] Processing job ${jobId}:`, {
        type: job.type,
        to: job.to,
 ...
 */
export async function simulateSendReminder(customerId: string, template: string): Promise<boolean> {
  console.info(`[Scheduler] Sending reminder to ${customerId} using template ${template}`);
