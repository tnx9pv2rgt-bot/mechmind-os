...
export async function extractTenantFromRequest(
  request: { headers: { get(name: string): string | null }; nextUrl?: { hostname: string; searchParams: { get(name: string): string | null } }; cookies?: { get(name: string): { value: string } | undefined } }
): Promise<{ value: string; type: 'subdomain' | 'domain' | 'header' | 'cookie' | 'param' } | null> {
