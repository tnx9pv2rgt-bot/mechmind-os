...
import { DigitalInspection, InspectionItem, STATUS_COLORS, CATEGORY_LABELS } from '@/types/inspection'
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-gray-500">
          Generato il {new Date(inspection.completedAt || inspection.startedAt).toLocaleDateString('it-IT')} 
          {' '}da {inspection.technicianName}
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <Card className={cn(
          "border-l-4",
          inspection.summary.urgentCount > 0 ? "border-l-red-500 bg-red-50" : "border-l-green-500 bg-green-50"
        )}>
        ...
        <Card className="border-l-4 border-l-blue-500 bg-blue-50">
          <CardContent className="p-4">
            <div className="text-3xl font-bold">€{inspection.summary.estimatedTotal.toFixed(0)}</div>
            ...
            <ul className="space-y-2">
              {needsAttentionItems.slice(0, 5).map(item => (
                <li key={item.id} className="flex items-center gap-2 text-red-700">
                  ...
                  {item.estimatedCost && (
                    <Badge variant="outline" className="ml-2">€{item.estimatedCost}</Badge>
                  )}
                </li>
              ))}
              ...
              >
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-gray-500">• {CATEGORY_LABELS[item.category]}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    {item.estimatedCost && (
