...
type RecorderState = 'idle' | 'recording' | 'paused' | 'preview' | 'uploading' | 'complete' | 'error'
...
const VEHICLE_POSITIONS: VehiclePosition[] = [
  { id: 'front_bumper', label: 'Front Bumper', labelIt: 'Paraurti Anteriore', startTime: 0, endTime: 10, icon: '↑', prompt: 'Start at the front bumper. Hold steady.' },
  { id: 'front_left', label: 'Front Left', labelIt: 'Anteriore Sinistro', startTime: 10, endTime: 15, icon: '↖', prompt: 'Move to front left corner.' },
  { id: 'left_side', label: 'Left Side', labelIt: 'Lato Sinistro', startTime: 15, endTime: 25, icon: '←', prompt: 'Continue along left side.' },
  { id: 'rear_left', label: 'Rear Left', labelIt: 'Posteriore Sinistro', startTime: 25, endTime: 30, icon: '↙', prompt: 'Move to rear left corner.' },
  { id: 'rear', label: 'Rear', labelIt: 'Posteriore', startTime: 30, endTime: 40, icon: '↓', prompt: 'Show rear bumper and trunk.' },
  { id: 'rear_right', label: 'Rear Right', labelIt: 'Posteriore Destro', startTime: 40, endTime: 45, icon: '↘', prompt: 'Move to rear right corner.' },
  { id: 'right_side', label: 'Right Side', labelIt: 'Lato Destro', startTime: 45, endTime: 55, icon: '→', prompt: 'Continue along right side.' },
  { id: 'front_right', label: 'Front Right', labelIt: 'Anteriore Destro', startTime: 55, endTime: 60, icon: '↗', prompt: 'Finish at front right corner.' },
]
...
export function VideoRecorder({ inspectionId, onUploadComplete }: VideoRecorderProps) {
  ...
  useEffect(() => {
    ...
    if (position) {
      ...
      if (progress < 10 && recordingTime >= position.startTime && recordingTime < position.startTime + 1) {
  ...
  const currentPosition = useMemo(() => 
    getCurrentPosition(recordingTime),
    [recordingTime]
  )
    ...
    <div className="absolute top-4 left-4 right-4 bg-black/70 backdrop-blur-sm rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white text-sm font-medium">360° Walkaround Guide</span>
        </div>
        <span className="text-white/80 text-xs">
          {currentPosition?.labelIt || 'Complete'}
        </span>
      </div>
    ...
    <div className="bg-gray-900 p-4 space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          {annotationMode ? 'Exit Annotation' : 'Add Hotspots'}
        </Button>
        
        {annotationMode && (
          <span className="text-xs text-white/60">
            Click on video to add notes
          </span>
        )}
      </div>
    ...
    <div className="absolute inset-0 bg-black/90 flex items-center justify-center z-50">
      <div className="w-full max-w-md p-8">
        ...
        <div className="flex justify-between text-sm text-white/60">
          ...
          <span>
            {uploadProgress
              ? formatDuration(uploadProgress.bytesProcessed)
              : '0 MB'}
            {' / '}
            {uploadProgress
              ? formatDuration(uploadProgress.totalBytes)
              : '0 MB'}
          </span>
        </div>
      </div>
    ...
    <div className="relative w-full h-full min-h-[600px] bg-black rounded-xl overflow-hidden">
          ...
          <div className="relative flex-1 bg-black">
            ...
            {annotationMode && (
              <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-blue-500/90 text-white px-4 py-2 rounded-full text-sm">
                Click on video to add a hotspot
              </div>
            )}
          </div>
      ...
      <Dialog open={showCancelDialog} onOpenChange={setShowCancelDialog}>
        <DialogContent className="bg-gray-900 border-gray-700 text-white">
          <DialogHeader>
            <DialogTitle>Discard Video?</DialogTitle>
            <DialogDescription className="text-gray-400">
              Are you sure you want to cancel? Your recorded video will be lost.
            </DialogDescription>
          </DialogHeader>
