...
import { useCreatePart, CreatePartInput } from '@/hooks/useApi';
...
export function PartDialog({
  ...
  const handleSubmit = async (data: Record<string, unknown>) => {
    ...
    try {
      await createPartMutation.mutateAsync(data as unknown as CreatePartInput);
    } catch {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className='sm:max-w-2xl max-h-[85vh] overflow-y-auto p-0 gap-0 bg-white/95 backdrop-blur-xl border-apple-border/50 rounded-[24px]'>
        ...
        <div className='p-6'>
          <PartForm onSubmit={handleSubmit} onCancel={() => setIsOpen(false)} />
        </div>
      </DialogContent>
    </Dialog>
  );
