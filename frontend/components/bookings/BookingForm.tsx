...
const mockVehicles: Record<
  string,
  Array<{ id: string; plate: string; model: string; year: number }>
> = {
...
export function BookingForm({ onSuccess, onCancel }: BookingFormProps) {
  ...
  const onSubmit = async (data: BookingFormData) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Errore creazione prenotazione: ${res.status}`);
      setStatus('success');
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} onKeyDown={handleKeyDown} className='space-y-5'>
