  ...
  return (
    ...
    >
      ...
      <CardContent className='space-y-4'>
        ...
        <Badge
          variant='outline'
          className={cn('flex items-center gap-1 w-fit', status.bgColor, status.color)}
        >
          {status.icon}
          {status.label}
        </Badge>
              ...
              >
                ...
                >
                  {claim.status === ClaimStatus.REJECTED
                    ? '—'
                    : formatCurrency(
                        (claim as unknown as Record<string, unknown>).approvedAmount as number
                      )}
                </div>
              </div>
            )}
        ...
        {claim.documents && claim.documents.length > 0 && (
          <div className='flex items-center gap-2 text-sm text-gray-600'>
            ...
            <span>
              {claim.documents.length} photo{claim.documents.length !== 1 ? 's' : ''} attached
            </span>
          </div>
        )}
