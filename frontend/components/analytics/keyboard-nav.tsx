      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed left-1/2 top-1/2 z-[61] w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl border border-white/10 bg-[#2f2f2f]/95 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.5)]"
          >
            {/* Header */}
            <div className="flex items-center justify-between border-b border-[#3a3a3a] px-6 py-4">
              ...
              <ul className="space-y-3">
                {SHORTCUTS.map((shortcut) => (
                  <li key={shortcut.key} className="flex items-center justify-between">
                    <span className="text-sm text-[#b4b4b4]">{shortcut.description}</span>
                    <kbd className="ml-4 flex h-7 min-w-[28px] flex-shrink-0 items-center justify-center rounded-lg border border-[#4e4e4e] bg-[#383838] px-2 font-mono text-xs text-white">
                      {shortcut.label}
                    </kbd>
                  </li>
                ))}
            ...
            {/* Footer */}
            <div className="border-t border-[#3a3a3a] px-6 py-3">
