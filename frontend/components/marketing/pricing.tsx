    ...
    <div className='flex items-center justify-center gap-3'>
      ...
      <button
        ...
        className={cn(
          ...
          period === 'annual'
            ? 'bg-[var(--brand)]'
            : 'bg-apple-border dark:bg-[var(--surface-active)]'
        )}
      >
        <span
          className={cn(
...
function PriceDisplay({ plan, period }: { plan: PricingPlan; period: BillingPeriod }): ReactNode {
  if (plan.monthlyPrice === null) {
    return (
      <div className='mb-6'>
        <span className='text-title-1 font-bold text-[var(--text-primary)] dark:text-[var(--text-on-brand)]'>
          Contattaci
        </span>
      </div>
    );
  ...
  return (
    <div className='mb-6'>
      <div className='flex items-baseline gap-1'>
        ...
        <span className='text-footnote text-[var(--text-tertiary)]'>/mese + IVA</span>
      </div>
      {showStrikethrough && (
...
function PlanCard({ plan, period }: { plan: PricingPlan; period: BillingPeriod }): ReactNode {
  return (
    <motion.div
    ...
    >
      ...
      <div className='mb-4'>
        <h3 className='text-title-2 font-bold text-[var(--text-primary)] dark:text-[var(--text-on-brand)]'>
          {plan.name}
        </h3>
      ...
      <ul className='mb-8 flex-1 space-y-3'>
        {plan.features.map(feature => (
          <li key={feature.label} className='flex items-start gap-3'>
            <Check
              className='mt-0.5 h-5 w-5 shrink-0 text-[var(--status-success)]'
              strokeWidth={2.5}
            />
    ...
    >
      <div className='mx-auto max-w-apple-wide px-6 lg:px-8'>
            ...
            <div className='flex flex-col items-center gap-6 sm:flex-row sm:justify-between'>
              <div className='text-center sm:text-left'>
                ...
                <p className='mt-2 max-w-lg text-callout text-[var(--text-tertiary)]'>
                  Rispondi alle chiamate con un assistente AI che parla italiano. Prenota
                  appuntamenti, verifica disponibilita, invia conferme SMS — tutto automatico.
        ...
        <motion.p
          ...
          animate={isInView ? 'visible' : 'hidden'}
          variants={fadeInUp}
          className='mt-8 text-center text-sm text-[var(--text-tertiary)]'
        >
          Tutti i prezzi sono IVA esclusa. IVA al 22% aggiunta in fattura.
        </motion.p>
      </div>
    </section>
  );
