...
const TEAM_SIZES: TeamSizeOption[] = [
  {
    ...
    name: 'Solo io',
    description: 'Gestisco tutto da solo',
  },
  {
    id: '2-5',
    icon: <Users className='h-5 w-5' />,
    name: '2-5 persone',
    description: 'Piccolo team',
  },
  {
    id: '6+',
    icon: <UsersRound className='h-5 w-5' />,
    name: '6+ persone',
    description: 'Team strutturato',
  },
];
...
const itemVariants = {
  hidden: { x: -16, scale: 0.97 },
  visible: { x: 0, scale: 1, transition: { type: 'spring', stiffness: 400, damping: 25 } },
};
    ...
    <div className='flex flex-col items-center gap-5'>
      <div className='text-center'>
        ...
        <p className='mt-1 text-[13px] text-[var(--text-secondary)]'>
          Adatteremo funzionalità e permessi
        </p>
      </div>
        ...
        {TEAM_SIZES.map(size => {
          ...
          return (
            ...
            >
              {isSelected && (
                <motion.div
                  className='absolute right-3 top-1/2 -translate-y-1/2 flex h-4 w-4 items-center justify-center rounded-full bg-emerald-500'
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
              ...
              <span className={isSelected ? 'text-emerald-400' : 'text-[var(--text-secondary)]'}>
                {size.icon}
              </span>
              <div>
                <span
