...
export function LocationComparison({ locations, metrics }: LocationComparisonProps) {
  ...
  const locationData = locations.map(location => {
    ...
    return {
      ...location,
      ...locationMetrics,
      totalVehicles: locationMetrics.carCount.inService + locationMetrics.carCount.waiting + locationMetrics.carCount.ready
    }
  })
  ...
  const rankings = {
    revenue: [...locationData].sort((a, b) => b.revenue[periodFilter] - a.revenue[periodFilter]).map(l => l.id),
  ...
  const getRankBadge = (locationId: string, metric: keyof typeof rankings) => {
    ...
    if (rank === locations.length - 1) return <AlertCircle className="h-4 w-4 text-status-urgent" />
    return null
  }
              ...
              {paginatedData.map((location, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-4 text-right">
                    <p className={cn(
                      'font-semibold',
                      rankings.revenue.indexOf(location.id) === 0 ? 'text-status-ready' : 'text-gray-900 dark:text-white'
                    )}>
                    ...
                    {rankings.revenue.indexOf(location.id) === locations.length - 1 && (
                      <span className="text-xs text-status-urgent font-medium">Necessita attenzione</span>
                    )}
                  </td>
                  <td className="px-4 py-4 text-right">
                  ...
                  <td className="px-4 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <p className="font-medium text-gray-900 dark:text-white">{formatCurrency(location.aro)}</p>
                    </div>
                  </td>
                  <td className="px-4 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <div className="w-16 h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                        <div 
                          className={cn(
                            'h-full rounded-full',
                            location.utilization >= 80 ? 'bg-status-ready' :
                            location.utilization >= 60 ? 'bg-status-warning' : 'bg-status-urgent'
                          )}
                      ...
                      <span className={cn(
                        'font-medium text-sm',
                        location.utilization >= 80 ? 'text-status-ready' :
                        location.utilization >= 60 ? 'text-status-warning' : 'text-status-urgent'
                      )}>
                  ...
                  <td className="px-4 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="font-medium text-gray-900 dark:text-white">{location.satisfaction}</span>
                      </div>
                    </div>
                  </td>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 dark:border-gray-700">
            <div className="text-sm text-gray-600 dark:text-gray-400">
              Mostrando {(currentPage - 1) * ITEMS_PER_PAGE + 1} - {Math.min(currentPage * ITEMS_PER_PAGE, filteredData.length)} di {filteredData.length}
            </div>
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-semibold text-gray-900 dark:text-white">
                {locationData.sort((a, b) => b.revenue[periodFilter] - a.revenue[periodFilter])[0]?.name}
              </p>
            </div>
          </div>
        </div>
