...
function generateTips(currentStep: number, formData: Record<string, unknown>): Suggestion[] {
  ...
  if (currentStep === 3) {
    tips.push({
      ...
      title: '🛡️ GDPR',
      message:
        'I consensi possono essere modificati in qualsiasi momento dalle impostazioni account',
    });
  }
              ...
              {suggestions.length === 0 && !isAnalyzing ? (
                ...
                suggestions.map(suggestion => (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -10 }}
                    className={cn('p-3 rounded-xl border text-sm', getBgColor(suggestion.type))}
                  >
