...
function IOSInput({
  ...
  return (
    <div className='relative group'>
      {Icon && (
        <div className='absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-[#636366] group-focus-within:text-blue-500 transition-colors'>
      ...
      <Input
        {...props}
        className={cn(
          'h-14 w-full rounded-2xl border-0 bg-white/60 dark:bg-[#353535]/60',
          'text-base text-gray-900 dark:text-[#ececec] placeholder:text-gray-400 dark:placeholder:text-[#6e6e6e]',
          ...
          {step === 1 && (
            ...
            >
              ...
              <div className='flex items-center gap-4 p-4 rounded-2xl bg-white/40 dark:bg-[#353535]/40 border border-white/60 dark:border-[#424242]/60'>
                ...
                <div>
                  <p className='font-medium text-gray-900 dark:text-[#ececec]'>App Authenticator</p>
          ...
          {step === 2 && (
            ...
            >
              <div className='text-center space-y-2'>
                ...
                <p className='text-gray-500 dark:text-[#636366]'>Usa la tua app authenticator</p>
              </div>
              ...
              <div className='flex items-center gap-2 p-3 rounded-xl bg-white/60 dark:bg-[#353535]/60 border border-white/60 dark:border-[#424242]/60'>
                <Key className='h-4 w-4 text-gray-400 dark:text-[#636366]' />
                <code className='text-sm flex-1 font-mono text-gray-700 dark:text-[#ececec]'>
                  {secret}
                </code>
                <button
                  onClick={copySecret}
                  className='p-2 rounded-lg hover:bg-white/50 dark:hover:bg-[#353535]/50 transition-colors'
                >
                  {copied ? (
                    <Check className='h-4 w-4 text-green-500' />
                  ) : (
                    <Copy className='h-4 w-4 text-gray-400 dark:text-[#636366]' />
                  )}
                </button>
              </div>
          ...
          {step === 3 && (
            ...
            >
              <div className='text-center space-y-2'>
                ...
                <p className='text-gray-500 dark:text-[#636366]'>Il tuo account è ora più sicuro</p>
              </div>

              <div className='p-5 rounded-2xl bg-amber-50/70 dark:bg-amber-950/30 border border-amber-200/70 dark:border-amber-800/50'>
                <div className='flex items-center gap-2 text-amber-800 mb-3'>
                  <AlertTriangle className='h-5 w-5' />
                  <span className='font-medium'>Codici di backup</span>
                </div>
                <p className='text-sm text-amber-700 mb-4'>
                  Salva questi codici in un posto sicuro. Servono se perdi l&apos;accesso
                  all&apos;app.
                </p>
                <div className='grid grid-cols-2 gap-2 mb-4'>
                  {backupCodes.map((code, i) => (
                    <code
                      key={i}
                      className='text-xs bg-white/80 dark:bg-[#353535]/80 p-2 rounded-lg font-mono text-center'
                    >
                      {code}
                    </code>
                  ))}
