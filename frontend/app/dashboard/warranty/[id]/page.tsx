...
export default function WarrantyDetailPage() {
  ...
  const status = statusConfig[warranty.status as WarrantyStatus] || { label: 'Unknown', color: 'bg-gray-100 text-gray-800' }
  ...
  const pendingClaims = warranty.claims?.filter(c => 
    c.status === ClaimStatus.SUBMITTED || c.status === ClaimStatus.UNDER_REVIEW
  ) || []
  const approvedClaims = warranty.claims?.filter(c => 
    c.status === ClaimStatus.APPROVED || c.status === ClaimStatus.PAID
  ) || []
  const rejectedClaims = warranty.claims?.filter(c => 
    c.status === ClaimStatus.REJECTED
  ) || []

  const canFileClaim = warranty.status !== WarrantyStatus.EXPIRED && 
            ...
            <CardHeader>
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className={cn("p-3 rounded-lg", status.color.replace('text-', 'bg-').replace('800', '100'))}>
              ...
              {warranty.status !== WarrantyStatus.EXPIRED && warranty.status !== WarrantyStatus.VOID && (
                <div className={cn(
                  ...
                  <Calendar className={cn(
                    ...
                    daysRemaining <= 60 ? "text-amber-600" :
                    "text-green-600"
                  )} />
                  <div>
                    <p className={cn(
      ...
      <Dialog open={claimDialogOpen} onOpenChange={setClaimDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Submit a new claim for this warranty
            </DialogDescription>
          </DialogHeader>
          <ClaimForm
