...
export default function AuditPage() {
  ...
  const getActionBadge = (action: string) => {
    const colors: Record<string, string> = {
      CREATE: 'bg-[var(--status-success-subtle)] text-[var(--status-success)] dark:bg-[var(--status-success)]/40/30 dark:text-[var(--status-success)]',
      UPDATE: 'bg-[var(--status-info-subtle)] text-[var(--status-info)] dark:bg-[var(--status-info)]/40/30 dark:text-[var(--status-info)]',
      DELETE: 'bg-[var(--status-error-subtle)] text-[var(--status-error)] dark:bg-[var(--status-error)]/40/30 dark:text-[var(--status-error)]',
      LOGIN: 'bg-[var(--brand)]/10 text-[var(--brand)] dark:bg-[var(--brand)]/40/30 dark:text-[var(--brand)]',
      LOGOUT: 'bg-[var(--surface-secondary)] dark:bg-[var(--surface-hover)] text-[var(--text-tertiary)] dark:text-[var(--text-secondary)]',
      EXPORT: 'bg-[var(--status-warning)]/20 text-[var(--status-warning)] dark:bg-[var(--status-warning)]/40/30 dark:text-[var(--status-warning)]',
    };
      ...
      <header>
        <div className='px-4 sm:px-8 py-5'>
          <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3'>
            <div>
              <h1 className='text-headline text-[var(--text-primary)] dark:text-[var(--text-primary)]'>Registro Audit</h1>
          ...
          <AppleCardContent>
            {entries.length > 0 ? (
                  ...
                  <table className='w-full'>
                    <thead>
                      <tr className='border-b border-[var(--border-default)]/30 dark:border-[var(--border-default)]'>
                        <th className='text-left py-3 px-4 text-xs font-medium text-[var(--text-primary)] dark:text-[var(--text-primary)]'>Data/Ora</th>
                        <th className='text-left py-3 px-4 text-xs font-medium text-[var(--text-primary)] dark:text-[var(--text-primary)]'>Utente</th>
                        <th className='text-left py-3 px-4 text-xs font-medium text-[var(--text-primary)] dark:text-[var(--text-primary)]'>Azione</th>
                        <th className='text-left py-3 px-4 text-xs font-medium text-[var(--text-primary)] dark:text-[var(--text-primary)]'>Risorsa</th>
                        <th className='text-right py-3 px-4 text-xs font-medium text-[var(--text-primary)] dark:text-[var(--text-primary)]'>Dettagli</th>
                      </tr>
                    </thead>
                    <tbody>
              ...
              <div className='text-center py-12'>
                <FileText className='w-12 h-12 text-[var(--text-tertiary)]/30 mx-auto mb-4' />
          ...
          {selectedEntry && (
            <div className='space-y-4 py-4'>
              <div className='grid grid-cols-2 gap-4'>
                <div>
                  <p className='text-footnote text-[var(--text-tertiary)] dark:text-[var(--text-secondary)]'>Data/Ora</p>
                ...
                <div>
                  <p className='text-footnote text-[var(--text-tertiary)] dark:text-[var(--text-secondary)]'>Utente</p>
                  <p className='text-body text-[var(--text-primary)] dark:text-[var(--text-primary)]'>{selectedEntry.userName}</p>
                </div>
                <div>
                  <p className='text-footnote text-[var(--text-tertiary)] dark:text-[var(--text-secondary)]'>Azione</p>
                  {getActionBadge(selectedEntry.action)}
                </div>
                <div>
                  <p className='text-footnote text-[var(--text-tertiary)] dark:text-[var(--text-secondary)]'>Risorsa</p>
                  <p className='text-body text-[var(--text-primary)] dark:text-[var(--text-primary)]'>
                    {selectedEntry.resource} ({selectedEntry.resourceId})
                  </p>
                </div>
                <div>
                  <p className='text-footnote text-[var(--text-tertiary)] dark:text-[var(--text-secondary)]'>IP</p>
                  <p className='text-body text-[var(--text-primary)] dark:text-[var(--text-primary)] font-mono text-sm'>
              ...
              {selectedEntry.details && (
                <div>
                  <p className='text-footnote text-[var(--text-tertiary)] dark:text-[var(--text-secondary)] mb-2'>Dettagli</p>
                  <pre className='p-4 bg-[var(--surface-secondary)]/50 dark:bg-[var(--surface-hover)] rounded-xl text-sm overflow-auto max-h-60 text-[var(--text-primary)] dark:text-[var(--text-primary)]'>
