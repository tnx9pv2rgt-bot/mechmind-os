...
function Badge({ color, children }: { color: 'green' | 'yellow' | 'blue'; children: React.ReactNode }) {
  const cls = {
    ...
    <div className="min-h-screen bg-[var(--surface-tertiary)] dark:bg-[var(--surface-primary)] px-6 py-12">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-[13px] text-[var(--text-tertiary)]">
              Aggiornato: 3 maggio 2026 · Contatto: security@nexo.it
            </p>
          </div>
          <a
            href="https://status.nexo.it"
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-1.5 rounded-lg border border-[var(--border-default)] bg-[var(--surface-secondary)] px-3 py-1.5 text-[13px] font-medium text-[var(--text-primary)] hover:bg-[var(--surface-secondary)]/80 transition-colors"
          >
        ...
        <p className="mt-4 text-[15px] text-[var(--text-secondary)] leading-relaxed">
          Nexo Gestionale è un ERP multi-tenant per officine automotive. Questa pagina documenta
          la nostra postura di sicurezza, le certificazioni di conformità e le politiche di
        ...
        <Section title="Crittografia">
          <Table
            ...
            rows={[
              ['PII a riposo', 'AES-256-CBC', 'Tutti i campi PII (nome, telefono, email, CF) via EncryptionService'],
        ...
        <Section title="Monitoring e Incident Response">
          <Table
            ...
            rows={[
              ['Sentry', 'Backend NestJS + Frontend Next.js — real-time error tracking, session replay'],
          ...
          <p className="mt-3 text-[13px]">
            <span className="font-medium text-[var(--text-primary)]">MTTR target P0:</span>{' '}
            &lt;1 ora. Vedi{' '}
        ...
        <Section title="SOC 2 Type II">
          ...
          <p className="text-[13px]">
            ...
            Contatta{' '}
            <a href="mailto:security@nexo.it" className="underline hover:text-[var(--text-primary)]">
              security@nexo.it
            </a>
            .
          </p>
        ...
        <Section title="Sub-Responsabili del Trattamento">
          ...
          <p className="text-[13px]">
            ...
            <a href="mailto:privacy@nexo.it" className="underline hover:text-[var(--text-primary)]">
        ...
        <Section title="Vulnerability Disclosure">
          <div className="rounded-xl border border-[var(--border-default)] bg-[var(--surface-secondary)]/40 p-4 space-y-3">
            ...
            <ol className="space-y-1.5 list-decimal list-inside text-[13px]">
              <li>
                Invia email a{' '}
                <a href="mailto:security@nexo.it" className="font-mono underline">
                  security@nexo.it
                </a>{' '}
                con oggetto <code className="bg-[var(--surface-secondary)] px-1 rounded">[VULN] descrizione breve</code>
              </li>
        ...
        <Section title="Contatti">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              { label: 'Security', email: 'security@nexo.it' },
              { label: 'Privacy / DPO', email: 'privacy@nexo.it' },
              { label: 'Generale', email: 'hello@nexo.it' },
              { label: 'DPO', email: 'dpo@nexo.it' },
            ].map(({ label, email }) => (
              <div
                key={label}
        ...
        <div className="mt-10 border-t border-[var(--border-default)] pt-6 flex items-center justify-between text-[12px] text-[var(--text-tertiary)]">
          ...
          <div className="flex gap-4">
            ...
            <Link href="/terms" className="hover:text-[var(--text-secondary)]">Termini</Link>
            <a href="mailto:security@nexo.it" className="hover:text-[var(--text-secondary)]">Sicurezza</a>
          </div>
        </div>
      </div>
    </div>
