...
/** GET /api/public/estimates/:token -> GET /v1/public/estimates/:token */
export async function GET(
  _req: NextRequest,
  { params }: { params: Promise<{ token: string }> }
): Promise<NextResponse> {
  const { token } = await params;
  return publicProxy(`v1/public/estimates/${token}`);
}

/** POST /api/public/estimates/:token -> POST /v1/public/estimates/:token/respond */
export async function POST(
  ...
  return publicProxy(`v1/public/estimates/${token}/respond`, 'POST', body);
}
