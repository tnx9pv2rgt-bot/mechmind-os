...
describe('RegisterPage', () => {
  ...
  describe('slug generation', () => {
    it('should auto-generate slug from shop name', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByPlaceholderText('Nome breve (es. officina-rossi)')).toHaveValue(
          'officina-rossi'
        );
      });
    });
    ...
    it('should allow manual slug override', async () => {
      ...
      render(<RegisterPage />);
      fireEvent.change(screen.getByPlaceholderText('Nome breve (es. officina-rossi)'), {
        target: { value: 'myslug' },
      });
      await waitFor(() => {
        expect(screen.getByPlaceholderText('Nome breve (es. officina-rossi)')).toHaveValue(
  ...
  describe('password visibility toggle', () => {
    it('should toggle password visibility', async () => {
      ...
      fireEvent.mouseDown(screen.getByLabelText('Mostra password'));
      await waitFor(() => {
        expect(screen.getByPlaceholderText('Password')).toHaveAttribute('type', 'text');
      });

      fireEvent.mouseDown(screen.getByLabelText('Nascondi password'));
      await waitFor(() => {
        expect(screen.getByPlaceholderText('Password')).toHaveAttribute('type', 'password');
      });
    });
  ...
  describe('field clearing on change', () => {
    ...
    it('should clear acceptTerms error when user checks', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByText('Devi accettare i termini e le condizioni')
