...
import WorkOrdersError from '@/app/dashboard/work-orders/error'

describe('WorkOrders Error boundary', () => {
  it('renders error heading', () => {
    render(<WorkOrdersError error={new Error('test')} reset={jest.fn()} />)
    expect(screen.getByText('Si è verificato un errore')).toBeInTheDocument()
  })

  it('renders custom error message', () => {
    render(<WorkOrdersError error={new Error('Connessione persa')} reset={jest.fn()} />)
    expect(screen.getByText('Connessione persa')).toBeInTheDocument()
  })

  it('renders fallback message when error.message is empty', () => {
    const err = new Error('')
    render(<WorkOrdersError error={err} reset={jest.fn()} />)
    expect(screen.getByText('Qualcosa è andato storto. Riprova.')).toBeInTheDocument()
  })

  it('renders Riprova button', () => {
    render(<WorkOrdersError error={new Error('err')} reset={jest.fn()} />)
    expect(screen.getByText('Riprova')).toBeInTheDocument()
  })

  it('calls reset when Riprova is clicked', () => {
    const mockReset = jest.fn()
    render(<WorkOrdersError error={new Error('err')} reset={mockReset} />)
    fireEvent.click(screen.getByText('Riprova'))
    expect(mockReset).toHaveBeenCalledTimes(1)
  })

  it('renders AlertCircle icon', () => {
    const { container } = render(<WorkOrdersError error={new Error('err')} reset={jest.fn()} />)
