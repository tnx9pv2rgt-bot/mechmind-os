...
jest.mock('@/components/ui/apple-button', () => ({
  AppleButton: ({
    ...
    React.createElement(
      'button',
      { onClick, disabled: disabled || loading, className, type, 'aria-label': al },
      icon,
      children
    ),
}));
...
describe('CustomersPage', () => {
  ...
  it('renders customer count from total', () => {
    mockCustomersData = {
      data: [makeCustomer(), makeCustomer({ id: 'cust-2', firstName: 'Luigi' })],
      total: 2,
    };
    render(<CustomersPage />);
  ...
  it('renders — when email/phone are missing', () => {
  ...
  it('renders pagination component', () => {
    const customers = Array.from({ length: 25 }, (_, i) =>
      makeCustomer({ id: `c${i}`, firstName: `A${i}` })
    );
